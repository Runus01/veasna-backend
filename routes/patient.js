...
router.get('/visit/:id', authenticateToken, requireRole(['any']), async (req, res) => {
  ...
  try {
    ...
    // Get referrals if consultation requires referral
    let referrals = [];
    if (visit.consultation_id && visit.require_referral) {
      const referralQuery = `
        SELECT 
          id,
          referral_date,
      ...
      `;
      const referralResult = await db.query(referralQuery, [visit.consultation_id]);
...
router.put('/:id', authenticateToken, requireRole(['any']), async (req, res) => {
    ...
    try {
      ...
      // Add last_updated_at and last_updated_by
